...
fn readItems(line : [] const u8, allocator : std.mem.Allocator) !std.ArrayList(u32) {
...
pub fn main() !void {
    ...
    while (counter < 20)
    {
        for (monkeys.items) |monkey, idx|
        {
            ...
            {
                const other = if (monkey.operation.self) old else monkey.operation.value;
                const new = @divFloor(switch (monkey.operation.op) {
                    .add => old + other,
                    .mul => old * other
                }, 3);
